...
class TestServerHTTPChannel(http.HTTPChannel):
    factory: "TestServerFactory"  # type: ignore
...
class Server:
    ...
    def start(self) -> None:
        ...
        ws_factory.server_instance = self  # type: ignore
...
class WebSocketProtocol(WebSocketServerProtocol):
    def onOpen(self) -> None:
        if self.factory is not None:
            server_instance = getattr(self.factory, "server_instance")
            if isinstance(server_instance, Server):
                for handler in server_instance._ws_handlers.copy():
                    server_instance._ws_handlers.remove(handler)
                    handler(self)
